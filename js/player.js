...
// 专辑名数组
let albums = ["我和我的祖国", "小蓓蕾组合", "十送红军"];
// 歌曲信息数组
let track_names = ["李谷一 - 我和我的祖国", "小蓓蕾组合 - 歌声与微笑", "中央乐团合唱团 - 十送红军"];
...
function playPause() {
  setTimeout(function () {
    if (audio.paused) {
      ...
      play_pause.querySelector(".iconfont").classList = "iconfont icon-zanting";
      album_cover.classList.add("active");
      audio.play();
    } else {
      player_track.classList.remove("active");
      play_pause.querySelector(".iconfont").classList = "iconfont icon-playfill";
...
function selectTrack(flag) {
  ...
  if (cur_index > -1 && cur_index < albums.length) {
    ...
    console.log(cur_track_name);
    // 设置音频路径
    audio.src = "src/" + cur_track_name + ".mp3";
    if (flag != 0) {
      // 当切换上一首,下一首时,自动播放
      audio.play();
      player_track.classList.add("active");
